...
if __name__ == "__main__":
    ...
    # Levenshtein Matcher (from training data)
    matcher = MITLLStringMatcher(algorithm='lev', text_normalizer = 'latin') # Initialize Levenshtein matcher
    matcher.fit(X_train,y_train)                                             # Fit matcher to training data
    matcher.save_model(os.path.join(exampledir,"data/output/models/english_socialmedia_lev.model"))
    ...
    # Levenshtein Matcher (from pre-trained model)
    matcher2 = MITLLStringMatcher(algorithm='lev',text_normalizer = 'latin')
    matcher2.load_model(os.path.join(exampledir,"data/output/models/english_socialmedia_lev.model"))

    posts2 = matcher2.predict_proba(X_test)

    if (posts2 == posts2).all(): logger.info("Levenshtein Test: Pass")
